...
def update_product(product, conn, cursor):
    ...
    # log_message(f"INFO: Updated product with ASIN {asin}", 0)
    # for image_link in image_links:
    #     cursor.execute(update_images_query, (image_link, asin))
...
def insert_data(product_list):
    ...
    insert_products = """
        INSERT INTO products (asin, title, description, occasion, product_dimensions)
        VALUES (?, ?, ?, ?, ?);
        SELECT SCOPE_IDENTITY();  -- Retrieve the ID of the newly inserted product
    """
    
    check_asin_exists = "SELECT COUNT(*) FROM products WHERE asin = ?;"
        ...
        for product in product_list:
            ...
            if asin and title:
                try:
                    ...
                    if exists:
                        log_message(f"INFO: ASIN {asin} already exists in the database. Updating product details.", 0)
                        update_product(product, conn, cursor)
                        continue

                    cursor.execute(insert_products, (asin, title, description, occasion, product_dimensions))
                    ...
                    if product_id_row:
                        ...
                        insert_dynamic_data(product_id, table_info, conn, cursor)
...
def insert_dynamic_data(product_id, table_info, conn, cursor):
    """ Insert the dynamic product details into the database."""
    cursor.execute("SELECT COLUMN_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_NAME = 'product_details';")
    existing_columns = [row[0] for row in cursor.fetchall()]

    try:
        for key in table_info.keys():
            if key not in existing_columns:
                alter_table_query = f"ALTER TABLE product_details ADD [{key}] NVARCHAR(MAX);"
                cursor.execute(alter_table_query)
                log_message(f"EXEC: Added missing column '{key}' to 'product_details' table.", 0)
        ...
        log_message(f"EXEC: Inserted data into 'product_details' for product_id {product_id}.", 0)
